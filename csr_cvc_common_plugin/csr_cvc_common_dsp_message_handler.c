...
void CsrCvcCommonDspMessageHandler( CvcPluginTaskdata *task ,uint16 id, Message message )
{
    switch(id)
    {
        case MESSAGE_FROM_KALIMBA:
        {
            ...
            {
                case CVC_CORE_READY:
                    PRINT(("CVC: core loaded, load app\n"));
                    KalimbaSendMessage(CVC_LOAD_INDEX, csrCvcGetAppIndex(), 0, 0, 0);
                    break;
                ...
                case CVC_READY_MSG:
                {
                    ...
                    csrCvcCommonStateMachineHandleEvent(cvc_ready_for_configuration, (cvc_event_t*)&event);
                }
                break;
