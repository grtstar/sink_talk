...
typedef struct audio_Tag
{
   ...
   Sink audio_sink;
   /*! The audio sink from parent */
   Sink audio_sink_parent;
...
{		
    ...
    switch(example_plugin_variant)
    {
        case CVSD_8K_1_MIC:
            ...
            {
                PRINT(("CSR_COMMON_EXAMPLE: one_mic_example_cvsd\n"));
                kap_file = "one_mic_example_cvsd/one_mic_example_cvsd.kap";
                CSR_COMMON_EXAMPLE->kap_type = 0;
            }
            break;
...
{
    AudioPluginSetMicBiasDrive(CSR_COMMON_EXAMPLE->voice_mic_params->mic_a, set);
    if(task->two_mic)
    {
        AudioPluginSetMicBiasDrive(CSR_COMMON_EXAMPLE->voice_mic_params->mic_b, set);
    }
}
...
static void connectAudio(const ExamplePluginTaskdata * const task)
{
    ...
    if(CSR_COMMON_EXAMPLE->audio_sink)
    {
        ...
        {
            r1 = (bool) StreamConnect(StreamSourceFromSink( CSR_COMMON_EXAMPLE->audio_sink_parent ),StreamKalimbaSink(2)); /* SCO->DSP */
            r2 = (bool) StreamConnect( StreamKalimbaSource(2), CSR_COMMON_EXAMPLE->audio_sink_parent ); /* DSP->SCO */
            PRINT(("Connect sco2 [%x] [%x]\n", r1, r2));
        }
...
void CsrExamplePluginConnect(const ExamplePluginTaskdata * const task, const AUDIO_PLUGIN_CONNECT_MSG_T * const connect_msg)
{
    ...
    if(isPluginVariantSbc(task))
    {
        ...
        if( CSR_COMMON_EXAMPLE->audio_sink_parent )
        {
            SourceConfigure(StreamSourceFromSink( CSR_COMMON_EXAMPLE->audio_sink_parent ),VM_SINK_SCO_SET_FRAME_LENGTH, BYTES_PER_MSBC_FRAME);
        }
    }
    ...
    if( CSR_COMMON_EXAMPLE->audio_sink_parent )
    {
        SourceConfigure(StreamSourceFromSink( CSR_COMMON_EXAMPLE->audio_sink_parent ),VM_SOURCE_SCO_METADATA_ENABLE,1);
    }
...
void CsrExamplePluginDisconnect(const ExamplePluginTaskdata * const task)
{
    ...
    if(CSR_COMMON_EXAMPLE->kap_type == 1)
    {
        StreamDisconnect(StreamSourceFromSink(CSR_COMMON_EXAMPLE->audio_sink_parent), StreamKalimbaSink(DSP_INPUT_PORT_FOR_AG2)); /* SCO->DSP */
        StreamDisconnect( StreamKalimbaSource(DSP_OUTPUT_PORT_FOR_AG2), CSR_COMMON_EXAMPLE->audio_sink_parent );                   /*DSP ->SCO */
    }
    if(CSR_COMMON_EXAMPLE->kap_type == 2)
    {
        StreamDisconnect(StreamSourceFromSink(CSR_COMMON_EXAMPLE->audio_sink_parent), StreamKalimbaSink(2)); /* SCO->DSP */
        StreamDisconnect( StreamKalimbaSource(2), CSR_COMMON_EXAMPLE->audio_sink_parent );                   /*DSP ->SCO */
    }
    
    CSR_COMMON_EXAMPLE->running = FALSE;
    CSR_COMMON_EXAMPLE->audio_sink = NULL;
    CSR_COMMON_EXAMPLE->audio_sink_parent = NULL;
