...
void sinkHandleIoCapabilityInd(const CL_SM_IO_CAPABILITY_REQ_IND_T* ind)
{
    ...
    if(can_pair)
    {
        ...
        /* Bonding only if in BA association state and remote BD address is random resolavable */
        if(sinkBroadcastAudioIsActive() && gapBaGetAssociationInProgress())
        {
             bonding = (key_dist == KEY_DISTRIBUTION_RANDOM);
        }
        DEBUG(("auth: Sending IO Capability \n"));
    ...
    {
        DEBUG(("auth: Rejecting IO Capability Req \n"));
