...
.MODULE $M.frame_sync.sco2_initialize;
   .CODESEGMENT FRAME_SYNC_SCO_INITIALIZE_PM;
   .DATASEGMENT DM;

$frame_sync.sco2_initialize:
   $push_rLink_macro;
    // reset packet count
...
.MODULE $M.frame_sync.sco2_port_handler2;
   ...
   // Size of last packet written to CBuffer
   .VAR $sco2_last_packet_size=0;
   // Amount of data in Port
   .VAR $sco2_port_data=0;
...
jp_do_again:
   ...
   r0 = M[r8 + $sco_pkt_handler.INPUT_PTR_FIELD];
   call $cbuffer.calc_amount_space;
   Null = r0 - M[$sco2_last_packet_size];
   if NEG jump $pop_rLink_and_rts;
   // Get Port
   r0 = M[r8 + $sco_pkt_handler.SCO_PORT_FIELD];
   call $cbuffer.get_read_address_and_size.its_a_port;
...
//
// *****************************************************************************
.MODULE $M.frame_sync.sco2_decode;
   .CODESEGMENT FRAME_SYNC_SCO_DECODE_PM;
   .DATASEGMENT DM;

   // Use to track position in CBuffer
   .VAR $next_packet_value2=0;

#ifdef DEBUG_ON  // Debug
   .VAR $plc_lost_packet2 = 0;
   .VAR $plc_missed_packet2 = 0;
   .VAR $plc_biterror_packet2 = 0;
   .VAR $sco_purge2= 0;
   .VAR $plc_fsco2 = 0;
   .VAR $plc_status2=0;
#endif

$frame_sync.sco2_decode:
   $push_rLink_macro;
...
jp_check_packet:
   ...
   r3 = M[$plc_lost_packet2];
   Null = r2 - 2;                     // Lost packet
   if Z r3 = r3 + M1;
   M[$plc_lost_packet2] = r3;

   r3 = M[$plc_missed_packet2];
   Null = r2 - 3;                     // Major Timing Error
   if Z r3 = r3 + M1;
   M[$plc_missed_packet2] = r3;

   r3 = M[$plc_biterror_packet2];      // Bit Error
   Null = r2 - 1;
   if Z r3 = r3 + M1;
   M[$plc_biterror_packet2] = r3;
#endif
...
//
// *****************************************************************************
.MODULE $M.frame_sync.sco2_decoder.pcm.initialize;
   .CODESEGMENT FRAME_SYNC_SCO_DECODER_PCM_INITIALIZE_PM;
   .DATASEGMENT DM;

   $frame_sync.sco2_decoder.pcm.initialize:
   rts;
.ENDMODULE;
...
.MODULE $M.frame_sync.sco2_decoder.pcm.validate;
   .CODESEGMENT FRAME_SYNC_SCO_DECODER_PCM_VALIDATE_PM;
   .DATASEGMENT DM;
 $frame_sync.sco2_decoder.pcm.validate:
...
// *****************************************************************************
// MODULE:
//    $frame_sync.sco2_decoder.pcm.process
//
// DESCRIPTION:
...
//
// *****************************************************************************
.MODULE $M.frame_sync.sco2_decoder.pcm.process;
...
$frame_sync.sco2_encode:
    ...
    // Call frame setup
    r1 = M[$M.sco2_timing.sco_data_ptr2];
...
jp_repeat:
   ...
   r7 = M[$M.sco2_timing.sco_data_ptr2];
   r0 = M[r7 + $sco_pkt_handler.ENCODER_BUFFER_FIELD]; // input
   call $cbuffer.calc_amount_data;
   r1 = M[r7 + $sco_pkt_handler.ENCODER_INPUT_SIZE_FIELD];
   NULL = r0 - r1;
   if NEG jump $pop_rLink_and_rts; // exit if we don't have a complete frame of input data
   // Ensure there is space for the output
   r0 = M[r7 + $sco_pkt_handler.SCO_OUT_BUFFER_FIELD]; // output
   call $cbuffer.calc_amount_space;
   r1 = M[r7 + $sco_pkt_handler.ENCODER_OUTPUT_SIZE_FIELD]; // encoded packet size
   NULL = r0 - r1;
   if NEG jump $pop_rLink_and_rts; // exit if we don't have space for a complete frame of output
...
//
// *****************************************************************************
.MODULE $M.sco2_timing.PortConnected;
...
port2_bt_address:
   ...
   M[$M.sco2_timing.wallclock_obj2 + $wall_clock.BT_ADDR_WORD1_FIELD] = r3;
   M[$M.sco2_timing.wallclock_obj2 + $wall_clock.BT_ADDR_WORD2_FIELD] = r4;
...
//
// *****************************************************************************
.MODULE $M.sco2_timing.sco2_params_handler;
...
params_handler:
   ...
   M[$M.sco2_timing.rcv_proc_trigger2]=r2;
   if NZ r2 = r7 - r2;
   ...
   M[$M.sco2_timing.sco_rcv_trigger2]=r2;
...
$sco2_timing.SyncClock2:
...
#endif
   // BT clk in usec
   r0 = M[&$M.sco2_timing.wallclock_obj2 + $wall_clock.ADJUSTMENT_VALUE_FIELD];
   r6 = r0 + r5;
   ...
   r0 = M[r8+$sco_pkt_handler.SCO_PARAM_TESCO_FIELD];
   if Z jump $pop_rLink_and_rts;
   NULL = M[&$M.sco2_timing.wallclock_obj2 + $wall_clock.ADJUSTMENT_VALUE_FIELD];
   if Z jump $pop_rLink_and_rts;
