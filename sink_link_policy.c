...
void linkPolicyUseMultiTalkSettings(Sink sink)
{
    if((SinkIsValid(sink)))
    {
        ConnectionSetLinkPolicy(sink, 1, mt_powertable);
    }
}
...
{
    LP_DEBUG(("RoleInd, status=%u  role=%s addr=(%x:%x:%lx)\n", ind->status, (ind->role == hci_role_master) ? "master" : "slave", ind->bd_addr.nap, ind->bd_addr.uap, ind->bd_addr.lap));
        ...
        if ((sinkDataIsMultipointEnabled()) && (linkPolicyNumberPhysicalConnections() > 1))
        {
...
#endif
            {
...
#endif
                {
                    ...
                    else if (mtIsOnlyParentConnect())
                    {
                        LP_DEBUG(("LP: MultiTalk: Child Disconnect, require master role (inverted)\n"));
                        requiredRole = hci_role_master;
                    }
                    else if (mtGetConnectDevices() == 2)
                    {
                        LP_DEBUG(("LP: MultiTalk: All Disconnect\n"));
                        requiredRole = hci_role_dont_care;
                    }
                    else
#endif
...
void linkPolicyCheckRoles(void)
{
    ...
    LP_DEBUG(("LP: linkPolicyCheckRoles: Hfp pri = %p, sec = %p, A2dp pri = %p, sec = %p\n", (void *)sink_pri, (void *)sink_sec, (void *)sink_a2dp_pri, (void *)sink_a2dp_sec));
